...
fn execute_execute_admin_msgs() {
    ...
    let res = create_dao(None, "exc_admin_msgs_create_dao", user_addr.clone());
    assert!(res.is_ok());
    let dao = res.unwrap();
    ...
    let res = Chain::process_msg("cw_core", "exc_admin_msgs_pause_dao_fail", &msg);
    assert!(res.is_err());

    let msg: CoreWasmMsg = WasmMsg::QueryMsg(cw_core::msg::QueryMsg::PauseInfo {});
    let res = Chain::process_msg("cw_core", "exc_admin_msgs_pause_dao_query", &msg).unwrap();
    ...
    let res = Chain::process_msgs("cw_core", "exc_admin_msgs_pause_dao", &msgs).unwrap();
...
fn execute_items() {
    ...
    // add item:
    let res = create_dao(Some(admin_addr.clone()), "exc_items_create_dao", admin_addr);
    assert!(res.is_ok());
    let dao = res.unwrap();
    ...
    let res = Chain::process_msg("cw_core", "exc_items_get", &msg).unwrap();
    ...
    let res = Chain::process_msgs("cw_core", "exc_items_set", &msgs).unwrap();
    ...
    let res = Chain::process_msgs("cw_core", "exc_items_rm", &msgs).unwrap();
    let res: GetItemResponse = serde_json::from_value(res[1]["data"].clone()).unwrap();
...
fn instantiate_with_no_admin() {
    ...
    let res = create_dao(None, "inst_dao_no_admin", user_addr);
    assert!(res.is_ok());
    let dao = res.unwrap();
...
fn instantiate_with_admin() {
    ...
    let staking_addr =
        &Chain::process_msg(voting_contract, "inst_admin_q_stake", &msg).unwrap()["data"];
    ...
    let res = Chain::process_msgs("cw20_stake", "inst_admin_q_val", &msgs).unwrap();
    ...
    let token_addr =
        &Chain::process_msg(voting_contract, "inst_admin_q_tok", &msg).unwrap()["data"];
    ...
    let res = Chain::process_msg(proposal_contract, "inst_admin_q_cfg", &msg).unwrap();
