...
use crate::msg::{
    ...
    FreezerAllowancesResponse, IsFrozenResponse, OwnerResponse, QueryMsg, StatusResponse,
};
use crate::state::{
...
pub fn query_mint_allowance(deps: Deps, address: String) -> StdResult<AllowanceResponse> {
    ...
    Ok(AllowanceResponse { address, allowance })
}
...
pub fn query_is_blacklisted(deps: Deps, address: String) -> StdResult<StatusResponse> {
    let status = BLACKLISTED_ADDRESSES
        ...
        .unwrap_or(false);
    Ok(StatusResponse { address, status })
}
