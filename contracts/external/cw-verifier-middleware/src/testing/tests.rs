...
use crate::{
    ...
    verify::{ec_pk_to_bech32_address, verify, ADDR_PREFIX},
};
...
fn test_generate_juno_addr_from_pk() {
    ...
    let generated_address = ec_pk_to_bech32_address(juno_pk, ADDR_PREFIX).unwrap();
...
fn test_verify() {
    ...
    let pub_key = "CiECK/U4YJxozQDJMTU2AtnjWFcywHj+eErS5PKc9Iamr6I=";
    ...
    // println!("pub key {}", wrapped_msg.public_key);

    let mut deps = mock_dependencies();
    let env = mock_env();
    let info = mock_info("creator", &[]);
    let err = verify(deps.as_mut(), env, info, wrapped_msg).err().unwrap();
    println!("{}", err);
}
