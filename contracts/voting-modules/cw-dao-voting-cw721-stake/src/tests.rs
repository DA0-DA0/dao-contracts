...
fn query_voting_power<T: Into<String>, U: Into<String>>(
    ...
    let result: cw_dao_core_interface::voting::VotingPowerAtHeightResponse =
        app.wrap().query_wasm_smart(contract_addr, &msg).unwrap();
    result.power
}
...
fn query_total_power_at_height<T: Into<String>>(
    ...
    let result: cw_dao_core_interface::voting::TotalPowerAtHeightResponse =
        app.wrap().query_wasm_smart(contract_addr, &msg).unwrap();
    result.power
}
...
fn test_info_query() {
    ...
    let info: cw_dao_core_interface::voting::InfoResponse = app
        .wrap()
    ...
    assert_eq!(
        info,
        cw_dao_core_interface::voting::InfoResponse {
