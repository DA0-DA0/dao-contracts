...
use cosmwasm_std::{
    to_binary, Addr, Binary, CosmosMsg, Deps, DepsMut, Empty, Env, MessageInfo, Response, StdError,
...
pub fn query_voting_power_at_height(
    ...
    to_binary(&cw_dao_core_interface::voting::VotingPowerAtHeightResponse { power, height })
}
...
pub fn query_total_power_at_height(deps: Deps, env: Env, height: Option<u64>) -> StdResult<Binary> {
    ...
    let power = TOTAL_STAKED_NFTS
        .may_load_at_height(deps.storage, height)?
        .unwrap_or_default();
    to_binary(&cw_dao_core_interface::voting::TotalPowerAtHeightResponse { power, height })
}
