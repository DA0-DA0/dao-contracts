...
use voting::{
    ...
    reply::{mask_proposal_execution_proposal_id, mask_proposal_hook_index, mask_vote_hook_index, pre_propose_module_instantiation_id, failed_pre_propose_module_hook_id},
    status::Status,
...
fn test_reply_hooks_mock() {
    ...
    let id = failed_pre_propose_module_hook_id();
    let err = reply(deps.as_mut(), env.clone(), prepropose_reply_msg).unwrap_err();
    assert!(matches!(err, ContractError::InvalidReplyID { id }));
